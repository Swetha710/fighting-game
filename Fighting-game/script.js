...
class Player {
  ...
  strike (player, enemy, attackDmg)
   {
    ...
    let msg = `${player.name} attacks ${enemy.name} for ${damageAmount}`
    return msg
  }
  ...
  heal (player) {
    ...
    let hpAmt = Math.floor(Math.random() * 4)+1
    // Add hpAmount to players health
    player.health += hpAmt
    //  Update the game and DOM with updateGame()
    updateGame(p1, p2, gameState)
    //  Return a message of 'player name heals for hpAmount HP'
    let msg = `${player.name} heals for ${hpAmt}`
    return msg
  }
}
...
let player1 = new Player('venice', 100, 10)
let player2 = new Player('lisa', 100, 10)
